...
A high-performance 32-point FFT hardware accelerator integrated into a PicoRV32 RISC-V SoC, targeting the SAED32 45nm technology library. The design combines six architectural optimisations to achieve a **~24× end-to-end latency reduction** over the baseline — from 61 µs down to ~2.5 µs per audio chunk, while reducing accelerator area by ~24% through datapath narrowing.
...
|---|---|---|
| FSM states | 13 (per-element SRAM R/W) | 5 (`INIT → LOAD → COMPUTE → STORE → FINISH`) |
| Compute architecture | Single butterfly, fully combinational, all via SRAM | 2× parallel, 4-stage pipelined, 24-bit register-file |
| Twiddle source | Read from SRAM (inside timed window) | Packed CSR registers (firmware preload before enable) |
...
| SRAM depth | 128 words (data + twiddles) | 64 words (data only) |
...
Total SoC area is **249,419 µm²**, well within the 596 × 596 µm = **355,362 µm² core budget** (70.2% utilisation). The 24-bit datapath narrowing reduced accelerator area by ~24% compared to the 32-bit variant (156,821 vs 206,976 µm²).
...
| `0x0300_004C+` | `MEM[0..63]` | SRAM data region (64 × 24-bit words, re/im interleaved) |
...
| `src/design/accelerator_mem.v` | Dual-interface SRAM: 32-bit narrow (CPU) + 64-bit wide paired (FFT), 24-bit internal storage with sign extension |
